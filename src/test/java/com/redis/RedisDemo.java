...
//        pool = new JedisPool(new JedisPoolConfig(), "127.0.0.1");
        pool = new JedisPool(new JedisPoolConfig(), "114.55.61.225");
...
//-----添加数据----------
        jedis.set("name", "于洋");//向key-->name中放入了value-->于洋
        System.out.println(jedis.get("name"));//执行结果：于洋
//-----修改数据-----------
        //1、在原来基础上修改
        jedis.append("name", "张三");   //很直观，类似map 将张三 append到已经有的value之后
        System.out.println(jedis.get("name"));//执行结果:于洋张三
        //2、直接覆盖原来的数据
        jedis.set("name", "李四");
        System.out.println(jedis.get("name"));//执行结果：李四
        //3、删除key对应的记录
        jedis.del("name");
         ...
         */
        jedis.mset("name", "王五", "jarorwar", "闵晓荣");
